...
| `lane-compilacao` | — | Workflow reutilizável `verify-android.yml` + script `lane-compilacao/verificar.sh` para análise estática pré-push (detecta imports faltando, ViewModels sem @HiltViewModel, UUID.randomUUID disperso, TOML quebrado) | (raiz) | `feature/lane-compilacao` | `.github/workflows/verify-android.yml`, `lane-compilacao/verificar.sh`, `.github/workflows/android-ci.yml` | 2026-09-17 | **merged (#62)** |
